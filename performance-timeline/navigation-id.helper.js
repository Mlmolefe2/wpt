...
let testResourceTiming = async (resourceTimingEntryId) => {
  ...
  let p = new Promise(resolve => {
    new PerformanceObserver((list) => {
      const entry = list.getEntries().find(e => e.name.includes('json_resource' + resourceTimingEntryId));
      if (entry) {
  ...
  const resp = await fetch('/performance-timeline/resources/json_resource' + resourceTimingEntryId + '.json');
  await p;
  return [navigationId];
}
...
let testElementTiming = async (elementTimingEntryId) => {
  ...
  let p = new Promise(resolve => {
    new PerformanceObserver((list) => {
      const entry = list.getEntries().find(e => e.entryType === 'element' && e.identifier === 'test-element-timing' + elementTimingEntryId);
      if (entry) {
        navigationId = entry.navigationId;
        resolve();
      }
...
function runNavigationIdTest(params, description) {
  ...
  promise_test(async t => {
    ...
    assert_true(
      ...
      'Navigation Ids should be the same at the initial load.');
